...
<xsl:template match="un:Unions">
    <html lang="ru" xml:lang="ru">
        <head>
            <title>Кружки, клубы, организации | Школа 30 | Ижевск</title>
        ...
        <body>
            ...
            <div id="main">
                <div id="header" class="html">
                    ...
                    <h1>Кружки, клубы, организации школы</h1>
                    ...
                    <div class="menu">
                        <xsl:call-template name="menu">
                            <xsl:with-param name="social-href" select="'http://www.school-30.com/api/unions'" />
                        </xsl:call-template>
                    </div>
                </div>
                <div id="docs">
                    <h4>найдена <xsl:value-of select="count(un:Union)" /> запись(ей).</h4>
                    <ul>
                        <xsl:for-each select="un:Union[not(un:name='Персонал школы')]">
                            <li>
                                <p><a href="{$ROOT}api/unions/{un:ID}/sources"><xsl:value-of select="un:name" /></a></p>
                            </li>
                        </xsl:for-each>
                    </ul>
                </div>
