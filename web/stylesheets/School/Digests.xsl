...
<xsl:include href="Digest.xsl" xml:base="." />

<xsl:variable name="ROOT" select="'../'" />

<xsl:template match="dig:Digests">
    <xsl:variable name="last" select="document(concat('../../api/digests/',dig:Digest[position() = 1]/dig:ID,'/sources'))/res:Resources" />
    <xsl:apply-templates select="$last" mode="digest" />
</xsl:template>
